...
#
include < iostream > #include < fstream > #include < string.h > #include < cstdlib > #include < map > #include < sstream >
...
void func_arquivo::cria_arquivo() {
 ...
 for (int i = 0; i < TAM; i++) {
  for (it = h.lista[i].begin(); it != h.lista[i].end(); it++) {
   ...
   arquivo << "\n" << it -> get_codigo() << "\t" << it -> get_idade() << "\t" << it -> get_nome();
   //cout << it->get_codigo();
...
void func_arquivo::inicia_data() {
 ...
 for (int i = 0; i < TAM; i++) {
  ...
  if (!arquivo.fail()) {
   ...
   cout << "Arquivo " << nome_arquivo << " importado com sucesso!!\n";
   arquivo.close();
  }
